...
# Aidan Cotsakis
# Started: 2023/03/14
# A racing game with a genetic algorithm to find the best line
...
class car(object):
	...
	def __init__(self, pos, angle, brain):
		...
		self.acceleration = 0.001 #game_units/tick^2
		self.deceleration = 0.001 #game_units/tick^2
		self.turningSpeed = 0.10 #rad/tick
	...
	def handleChechpoint(self):
		...
		if self.checkpoint == len(checkpoints) and not self.done:
			...
			if delX + delY < 0.5:
				global maxBrain, drawBackground, fps
...
def playerInput():
	...
	if keys[pygame.K_s]:
		movementArr[0] -= 1
	if keys[pygame.K_a] and player.velocity < 0:
		movementArr[1] -= 1
	elif keys[pygame.K_a]:
		movementArr[1] += 1
	if keys[pygame.K_d] and player.velocity < 0:
		movementArr[1] += 1
	elif keys[pygame.K_d]:
		movementArr[1] -= 1

	player.inputs(movementArr)
...
while not quit:
	...
	# update screen
	if drawBackground or generation % generationViewer == 0:
		draw()
